...
WHERE
	...
	AND last_name NOT LIKE '%Knott%'
;
-- 5. (Utiliza operadores lógicos) Nombre que empiece con la letra A y el apellido con que no empiece con la letra B
SELECT
	CONCAT(first_name, ' ', last_name) AS full_name
FROM
	customer
...
WHERE
	(p.amount) > 10
	AND
	r.rental_date BETWEEN '2007-02-15' AND '2007-02-20'
ORDER BY
	valor_total DESC
;
...
SELECT
	...
	SUM(p.amount) AS monto_acumulado
FROM
	staff s
...
SELECT
	...
	COUNT(f.film_id) AS total_pelis
FROM
	actor a
INNER JOIN film_actor f USING (actor_id)
GROUP BY
