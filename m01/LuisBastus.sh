...
for user in "$@"; do
    sudo userdel -r "$user" 2>/dev/null && echo "Usuario $user eliminado."
done
sudo groupdel "$group1" 2>/dev/null && echo "Grupo $group1 eliminado."
sudo groupdel "$group2" 2>/dev/null && echo "Grupo $group2 eliminado."
